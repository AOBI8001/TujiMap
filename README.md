...
[在线体验](https://tujimap.aobi.qzz.io/) · [更新日志](CHANGELOG.md) · [性能验证](docs/performance-1.3.md) · [项目仓库](https://github.com/AOBI8001/TujiMap)
...
批次响应包含服务端墙钟耗时、上游调用数、累计排队时间与累计上游时间。累计值包含并行任务，不能相加当作用户实际等待时间。自动回归测试使用真实 workerd 运行时和模拟上游校验生命周期、QPS、缓存、重试与分天约束。
