...
[![Version](https://img.shields.io/badge/version-1.0.0-168766)](https://github.com/AOBI8001/TujiMap) [![React](https://img.shields.io/badge/React-19-2868e8)](https://react.dev/) [![TypeScript](https://img.shields.io/badge/TypeScript-5-3178c6)](https://www.typescriptlang.org/) [![License](https://img.shields.io/badge/license-MIT-17211e)](LICENSE)

**随心成行，按途索迹。** 途迹把“发现景点、筛选景点、安排天数、生成真实导航”整合到同一张地图中，帮助用户得到少折返、可理解、可继续编辑的城市游览方案。

[在线体验](https://tujimap.aobi.qzz.io/) · [项目仓库](https://github.com/AOBI8001/TujiMap)
...
V1.0 覆盖 100 个国内城市，支持 1–7 天行程、多个住处、开放路线与闭合路线，并提供驾车、公共交通和纯步行三种真实道路导航。
...
每天的地点被转换为连续路段：`起点 → 景点 1 → … → 景点 n → 终点`。驾车、公共交通和纯步行三种模式可并行计算；同一模式内的路段按顺序请求。地图只绘制高德返回的真实道路折线，不使用跨江、穿楼或离开道路的直线作为伪导航替代。
...
| 优化对象 | 实现方式 | 解决的问题 |
| --- | --- | --- |
| 地图首屏 | 先使用城市基线进入地图，再在后台补齐 POI | 慢网络下避免进入页长时间空白 |
| 搜索输入 | 480 ms 防抖、30 分钟浏览器缓存 | 减少重复关键词请求与输入抖动 |
| 路线提交 | 250 ms 合并更新、取消过期请求 | 连续编辑时避免旧结果覆盖新结果 |
| 接口并发 | 每类高德服务独立排队，间隔 420 ms | 将单实例请求控制在约 2.38 QPS |
| 异常恢复 | 可重试错误进行一次带随机抖动的重试 | 缓解瞬时限流、超时与上游繁忙 |
| 路线缓存 | 前端路段缓存、服务端内存缓存与 CDN 缓存 | 切换交通方式或局部编辑时复用结果 |
| 图片加载 | 进入地图后以 3 路受控并发预加载全部卡片图片 | 滑动卡片时减少图片等待与闪烁 |
...
V1.0 专注于城市中心城区的景点选择和路线组织，不包含账号登录、酒店预订、门票支付、营业时间、实时票价或行程社交功能。
